...
public class MatiereDAO extends DAO<Matiere> {
  ...
  public boolean add(Matiere obj) {
            //Requête pour add
    String query =  "INSERT INTO matiere (id,note,appreciation,idenseignement) VALUES ("+ obj.getId() +
                ","+obj.getNote()+
                ",'"+obj.getAppreciation()+
                "',"+obj.getIdenseignement()+")";
  ...
  public boolean update(Matiere obj) {
      ...
      String query =  "UPDATE matiere SET id="+obj.getId()+",note="+obj.getNote()+",appreciation='"+obj.getAppreciation()+
                ",idenseignement="+obj.getIdenseignement()+" WHERE (id="+obj.getId()+")";
        try {
            this.connect.executeUpdate(query);
        } catch (SQLException ex) {
